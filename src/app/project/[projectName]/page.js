...
export async function generateStaticParams() {
  const paths = imageList.map((image) => ({
    projectName: slugify(image.project_name),
  }));
      ...
      <div className="w-full lg:w-1/2">
        <div className="flex items-center">
          ...
          {currentProject.is_personal_project && (
            <span className="bg-green-500 text-white px-2 py-1 ml-2 rounded-full">
              Personal
            </span>
          )}
          ...
          {/* badge */}
          {!currentProject.is_personal_project && (
            <span className="bg-blue-500 text-white px-2 py-1 ml-2 rounded-full">
              Client
            </span>
          )}
        </div>
        ...
        <div className="mt-4">
          ...
          {currentProject.technologies.map((tech, index) => (
            ...
            <span
              key={index}
              className="bg-secondary text-whte px-2 py-1 mr-2 rounded-full"
            >
              {tech}
            </span>
          ))}
